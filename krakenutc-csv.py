...
def getsavedcsvfiles():
    try:
        ...
        timeendings = ("1", "5", "30", "60", "240", "1440", "10080", "21600")
...
def main(runoncenopromts = False):
    # to read more API documentation here https://docs.kraken.com/rest/#tag/Market-Data/operation/getTickerInformation
    interval = "5"   # change 1, 5, 30, 60, 240, 1440, 10080, 21600 minutes
    ...
    try:
        ...
        print("no coin pair given. Usage is follows: python3 krakrenutc-csv.py BTCUSD EHTUSD")
        ...
        if len(listofcollections) == 0:  # no values given by user, no files in directory - use deault coin and interval
            ...
            listofcollections = ["ADAUSD"]  # some default values if no csv files given
    ...
    while True:   # forever loop
        for api_symbol, interval in zip(listofcollections, listofintervals):
            ...
            URL = "https://api.kraken.com/0/public/OHLC?pair=" + api_symbol + "&interval=" + interval + "&since=" + str(startunix)
            ...
            data = r.json()
            print(data["result"][api_symbol])
            data = data["result"][api_symbol][:-1]  #get data section, reverse, remove last column (waiting for the final data of the time period)
            ...
            for gg in data:
                if startunix < int(gg[0]):    #  time.asctime(time.gmtime(now))  include only new entries, reduce number of zeros in unix time
                    collection.append(insertcoinprice(int(gg[0]),time.asctime(time.gmtime(int(gg[0]))),gg[4],gg[6])) #add more if needed after time gg["high"]
                else:
